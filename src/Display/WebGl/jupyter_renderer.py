...
try:
    ...
    from ipywidgets import HTML, HBox, VBox, Checkbox, Button, Layout, Dropdown
    import numpy as np
except ImportError:
...
class Axes(Helpers):
    ...
    def __init__(self, bb_center, length=1, width=3):
        ...
        for vector, color in zip(([length, 0, 0], [0, length, 0], [0, 0, length]), ('red', 'green', 'blue')):
            self.axes.append(
                LineSegments2(
                    LineSegmentsGeometry(positions=[[self.center, _shift(self.center, vector)]]),
...
class JupyterRenderer:
    def __init__(self,
        ...
        * parallel: optional, False by default. If set to True, meshing runs in parallelized mode.
        """
        self._default_shape_color = default_shape_color or format_color(166, 166, 166)
        self._default_edge_color = default_edge_color or format_color(32, 32, 32)  # cark gray
        self._pick_color = format_color(232, 176, 36)  # orange
        ...
        self._toggle_shp_visibility_button = self.create_button("Hide/Show", "Toggle Shape Visibility",
                                                                 True, self._toggle_shape_visibility)
    ...
    def _toggle_shape_visibility(self, *kargs):
        self.clicked_obj.visible = not self.clicked_obj.visible

    def _compute_properties(self, *kargs):
        self.html.value = "<b>Properties:<b><br>%s" % dir(self._current_shape_selection)
        
    def toggle_axes_visibility(self, change):
        self.axes.set_visibility(_bool_or_new(change))

    def toggle_grid_visibility(self, change):
        self.grid.set_visibility(_bool_or_new(change))
    ...
    def DisplayShape(self,
        ...
        if vertex_color is None:
            vertex_color = self._default_edge_color
        ...
        # or a 1d element such as edge or wire ?
            ...
            result = self.AddShapeToScene(shp, shape_color, render_edges,
                                         edge_color, vertex_color, quality, transparency, opacity)
            output.append(result)
    ...
    def Display(self, rotation=None):
    ...
    def _reset(self, *kargs):
        ...
        self._camera.position = self._scale((1, 1, 1))
